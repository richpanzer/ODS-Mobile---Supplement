// Client side database variables
var dbInfo = new Array();
dbInfo['name']    = 'My Dietary Supplement'; // Short Name
dbInfo['ver']     = '1.5'; // Database version
dbInfo['display'] = 'My Dietary Supplements'; // Display Name
dbInfo['size']    = 65536; // Maximum size

// Google Analytics
var GACode = 'UX-AAAAAA';

// Error messages...
var noUsersError = 'No profiles exist';
var noSupplementsError = 'No supplements to list';
var addSupplementHeadingDefault = 'Add Dietary Supplement';
var addSupplementHeadingUserStart = 'Add to Profile: ';
var addSupplementHeadingUserEnd = '';
var supNotesBlank = 'No notes were made.';