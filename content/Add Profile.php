 <div class="inner-has-bottomtools">
  <div class="toolbar">
      <h1>Add Profile</h1>
      <a class="pop blueButton" href="#Profiles">Profiles</a>
      <a class="button submitUser pop" href="#Profile">Save</a>
  </div>
  <form id="saveUser" method="post">
    <ul class="rounded">
      <li><input type="text" placeholder="Enter Profile Name" name="profile_name" id="profile_name" /></li>
    </ul>
  </form>
  <div class="content">
    <p><a href="#Add_Dietary_Supplement" class="sumbitUserSubmitProfile whiteButton pop">Add Dietary Supplement</a></p>
  </div>
</div>
<?php include PATH_TXT . 'includes/toolbar.php'; ?>